...
export async function generateMetadata({
  ...
  const lesson = getEnglishLesson(slug)
...
export default async function EnglishLessonPage({
  ...
  const customer = await requireCustomer(
    `/curso-ingles-basico/aulas/${lesson.slug}`,
  )

  const enrolled = await isEnrolledInCourse(customer.id, 'ingles-basico')
  if (!enrolled) redirect('/curso-ingles-basico')

  const completedLessons = await getCompletedLessonSlugs(customer.id)

  const currentIndex = englishCourseLessons.findIndex(
    (item) => item.slug === lesson.slug,
  )
  const previousLesson = englishCourseLessons[currentIndex - 1]
  const nextLesson = englishCourseLessons[currentIndex + 1]
  const lessonSlugs = englishCourseLessons.map((item) => item.slug)
      ...
      <div className="mx-auto max-w-7xl px-4 py-8">
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[270px_1fr]">
          <aside className="order-2 space-y-4 lg:order-none lg:sticky lg:top-36 lg:self-start">
            ...
            <nav className="rounded-2xl border border-blue-200 bg-white/85 p-3 shadow-sm lg:max-h-[62vh] lg:overflow-y-auto">
                ...
                <div className="mt-2 rounded-lg" key={module.slug}>
                  ...
                  {module.lessons.map((item) => (
                    <a
                      ...
                      href={`/curso-ingles-basico/aulas/${item.slug}`}
                      key={item.slug}
                    >
                      {item.title}
                    </a>
          ...
          <article className="order-1 space-y-6 lg:order-none">
            <header className="rounded-3xl border border-blue-200 bg-gradient-to-br from-white via-blue-50 to-indigo-50 p-6 shadow-sm sm:p-8">
            ...
            {lessonVideo ? (
              <section className="overflow-hidden rounded-2xl border border-blue-200 bg-black shadow-sm">
            ...
            <section className="rounded-2xl border border-blue-200 bg-white/85 p-5 shadow-sm sm:p-6">
            ...
            <section className="grid gap-4">
              {lesson.keyPoints.map((point, index) => (
                <div
                  className="rounded-2xl border border-blue-200 bg-white/85 p-5 shadow-sm sm:p-6"
                  key={point.title}
                >
                  <div className="flex items-start gap-4">
                    <span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-blue-100 text-sm font-black text-blue-800">
                      {index + 1}
                    </span>
                    <div>
            ...
            <nav className="grid gap-3 border-t border-blue-200 pt-6 sm:grid-cols-2">
              {previousLesson ? (
                <a
                  className="rounded-2xl border border-blue-200 bg-white/85 p-4 text-sm font-bold transition hover:border-blue-400 hover:bg-blue-50"
                  href={`/curso-ingles-basico/aulas/${previousLesson.slug}`}
                >
              ...
              {nextLesson ? (
                <a
                  className="rounded-2xl border border-blue-200 bg-blue-700 p-4 text-sm font-bold text-white transition hover:bg-blue-800 sm:text-right"
                  href={`/curso-ingles-basico/aulas/${nextLesson.slug}`}
                >
                  <span className="flex items-center justify-end gap-1 text-xs text-blue-100">
                ...
                <a
                  ...
                  href="/curso-ingles-basico/prova-final"
                >
