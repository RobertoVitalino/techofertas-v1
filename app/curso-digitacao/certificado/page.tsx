...
export default async function TypingCertificateReturnPage({
  searchParams,
}: {
  searchParams: Promise<{ status?: string }>
}) {
  const { status: statusParam } = await searchParams
  const customer = await requireCustomer('/curso-digitacao/certificado')
  const certificateStatus = await getCertificateStatus(customer.id, 'digitacao')
      ...
      <div className="mx-auto max-w-2xl px-4 py-16">
        <section className="rounded-3xl border border-rose-200 bg-white/85 p-8 text-center shadow-sm">
          {certificateStatus.state === 'issued' ? (
            ...
            <>
              <span className="mx-auto grid h-14 w-14 place-items-center rounded-full bg-rose-700 text-white">
                <Clock3 size={28} />
              </span>
              <h1 className="mt-5 text-2xl font-black text-rose-950">
          ...
          <a
            className="mt-8 inline-block text-sm font-bold text-rose-700 hover:text-rose-900"
