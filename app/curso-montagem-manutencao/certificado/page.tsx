...
export default async function HardwareCertificateReturnPage({
  searchParams,
}: {
  searchParams: Promise<{ status?: string }>
}) {
  const { status: statusParam } = await searchParams
  const customer = await requireCustomer('/curso-montagem-manutencao/certificado')
  const certificateStatus = await getCertificateStatus(customer.id, 'montagem-manutencao')
  ...
  return (
    <main className="site-light-theme min-h-screen">
      ...
      <div className="mx-auto max-w-2xl px-4 py-16">
        <section className="rounded-3xl border border-orange-200 bg-white/85 p-8 text-center shadow-sm">
          {certificateStatus.state === 'issued' ? (
            <>
              <CertificatePurchaseTracker
                valueCents={courseRegistry['montagem-manutencao'].priceCents}
            ...
            <>
              <span className="mx-auto grid h-14 w-14 place-items-center rounded-full bg-orange-700 text-white">
                <Clock3 size={28} />
              </span>
              <h1 className="mt-5 text-2xl font-black text-orange-950">
                Processando pagamento
              </h1>
              <p className="mt-2 text-sm leading-6 text-slate-600">
                Assim que a confirmação chegar do Mercado Pago, seu certificado
                aparecerá automaticamente aqui e na página do curso. Isso
                costuma levar apenas alguns instantes.
              </p>
            </>
          )}

          <a
            className="mt-8 inline-block text-sm font-bold text-orange-700 hover:text-orange-900"
