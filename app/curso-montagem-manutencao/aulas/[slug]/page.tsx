...
export async function generateMetadata({
  ...
  const lesson = getHardwareLesson(slug)
...
export default async function HardwareLessonPage({
  ...
  const previousLesson = hardwareCourseLessons[currentIndex - 1]
  const nextLesson = hardwareCourseLessons[currentIndex + 1]
  const lessonSlugs = hardwareCourseLessons.map((item) => item.slug)
      ...
      <div className="mx-auto max-w-7xl px-4 py-8">
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[270px_1fr]">
          <aside className="order-2 space-y-4 lg:order-none lg:sticky lg:top-36 lg:self-start">
            ...
            <nav className="rounded-2xl border border-orange-200 bg-white/85 p-3 shadow-sm lg:max-h-[62vh] lg:overflow-y-auto">
                ...
                <div className="mt-2 rounded-lg" key={module.slug}>
                  ...
                  {module.lessons.map((item) => (
                    <a
                      ...
                      href={`/curso-montagem-manutencao/aulas/${item.slug}`}
                      key={item.slug}
                    >
                      {item.title}
                    </a>
          ...
          <article className="order-1 space-y-6 lg:order-none">
            <header className="rounded-3xl border border-orange-200 bg-gradient-to-br from-white via-orange-50 to-stone-50 p-6 shadow-sm sm:p-8">
              <div className="flex flex-wrap items-center gap-3 text-xs font-bold text-orange-700">
            ...
            {lessonVideo ? (
              <section className="overflow-hidden rounded-2xl border border-orange-200 bg-black shadow-sm">
            ...
            <section className="rounded-2xl border border-orange-200 bg-white/85 p-5 shadow-sm sm:p-6">
            ...
            <section className="grid gap-4">
              {lesson.keyPoints.map((point, index) => (
                <div
                  className="rounded-2xl border border-orange-200 bg-white/85 p-5 shadow-sm sm:p-6"
                  key={point.title}
                >
                  <div className="flex items-start gap-4">
                    <span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-orange-100 text-sm font-black text-orange-800">
                      {index + 1}
                    </span>
                    <div>
            ...
            <section className="rounded-2xl border border-stone-200 bg-stone-50 p-5 sm:p-6">
              <div className="flex items-center gap-3">
                <span className="grid h-10 w-10 place-items-center rounded-xl bg-stone-700 text-white">
                  <ListChecks size={21} />
                </span>
                <div>
              ...
              <ol className="mt-5 space-y-3">
                {lesson.activity.steps.map((step, index) => (
                  <li className="flex items-start gap-3 text-sm leading-6 text-slate-700" key={step}>
                    <span className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-stone-200 text-xs font-black text-stone-900">
                      {index + 1}
                    </span>
                    {step}
                  </li>
            ...
            <nav className="grid gap-3 border-t border-orange-200 pt-6 sm:grid-cols-2">
              {previousLesson ? (
                <a
                  className="rounded-2xl border border-orange-200 bg-white/85 p-4 text-sm font-bold transition hover:border-orange-400 hover:bg-orange-50"
                  href={`/curso-montagem-manutencao/aulas/${previousLesson.slug}`}
                >
              ...
              {nextLesson ? (
                <a
                  className="rounded-2xl border border-stone-200 bg-orange-700 p-4 text-sm font-bold text-white transition hover:bg-orange-800 sm:text-right"
                  href={`/curso-montagem-manutencao/aulas/${nextLesson.slug}`}
                >
                  <span className="flex items-center justify-end gap-1 text-xs text-orange-100">
                ...
                <a
                  className="rounded-2xl border border-stone-200 bg-stone-700 p-4 text-sm font-bold text-white sm:text-right"
                  href="/curso-montagem-manutencao/prova-final"
                >
                  <span className="text-xs text-stone-100">Curso concluído</span>
