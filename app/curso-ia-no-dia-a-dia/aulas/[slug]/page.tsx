...
export default async function AILessonPage({
  ...
  const customer = await requireCustomer(
    `/curso-ia-no-dia-a-dia/aulas/${lesson.slug}`,
  )

  const enrolled = await isEnrolledInCourse(customer.id, 'ia-no-dia-a-dia')
  if (!enrolled) redirect('/curso-ia-no-dia-a-dia')

  const completedLessons = await getCompletedLessonSlugs(customer.id)

  const currentIndex = iaCourseLessons.findIndex(
    (item) => item.slug === lesson.slug,
  )
  const previousLesson = iaCourseLessons[currentIndex - 1]
  const nextLesson = iaCourseLessons[currentIndex + 1]
  const lessonSlugs = iaCourseLessons.map((item) => item.slug)
      ...
      <div className="mx-auto max-w-7xl px-4 py-8">
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[270px_1fr]">
                ...
                <div className="mt-2 rounded-lg" key={module.slug}>
                  ...
                  {module.lessons.map((item) => (
                    <a
                      ...
                      href={`/curso-ia-no-dia-a-dia/aulas/${item.slug}`}
                      key={item.slug}
                    >
                      {item.title}
                    </a>
          ...
          <article className="order-1 space-y-6 lg:order-none">
            <header className="rounded-3xl border border-violet-200 bg-gradient-to-br from-white via-violet-50 to-fuchsia-50 p-6 shadow-sm sm:p-8">
            ...
            <section className="rounded-2xl border border-fuchsia-200 bg-fuchsia-50 p-5 sm:p-6">
              <div className="flex items-center gap-3">
                <span className="grid h-10 w-10 place-items-center rounded-xl bg-fuchsia-700 text-white">
                  <ListChecks size={21} />
                </span>
                <div>
              ...
              <ol className="mt-5 space-y-3">
                {lesson.activity.steps.map((step, index) => (
                  <li className="flex items-start gap-3 text-sm leading-6 text-slate-700" key={step}>
                    <span className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-fuchsia-200 text-xs font-black text-fuchsia-900">
                      {index + 1}
                    </span>
                    {step}
                  </li>
            ...
            <nav className="grid gap-3 border-t border-violet-200 pt-6 sm:grid-cols-2">
              {previousLesson ? (
                <a
                  ...
                  href={`/curso-ia-no-dia-a-dia/aulas/${previousLesson.slug}`}
                >
              ...
              {nextLesson ? (
                <a
                  ...
                  href={`/curso-ia-no-dia-a-dia/aulas/${nextLesson.slug}`}
                >
