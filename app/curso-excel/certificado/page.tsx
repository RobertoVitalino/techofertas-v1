...
export default async function ExcelCertificateReturnPage({
  searchParams,
}: {
  searchParams: Promise<{ status?: string }>
}) {
  const { status: statusParam } = await searchParams
  const customer = await requireCustomer('/curso-excel/certificado')
  const certificateStatus = await getCertificateStatus(customer.id, 'excel')
  ...
  return (
    <main className="site-light-theme min-h-screen">
      ...
      <div className="mx-auto max-w-2xl px-4 py-16">
        <section className="rounded-3xl border border-violet-200 bg-white/85 p-8 text-center shadow-sm">
          {certificateStatus.state === 'issued' ? (
            <>
              <CertificatePurchaseTracker
                valueCents={courseRegistry.excel.priceCents}
            ...
            <>
              <span className="mx-auto grid h-14 w-14 place-items-center rounded-full bg-violet-700 text-white">
                <Clock3 size={28} />
              </span>
              <h1 className="mt-5 text-2xl font-black text-violet-950">
          ...
          <a
            className="mt-8 inline-block text-sm font-bold text-violet-700 hover:text-violet-900"
            href="/curso-excel"
          >
            Voltar para o curso
          </a>
