...
const COURSE_SLUG = 'excel'

export const metadata = {
  title: 'Curso de Excel',
  description: `Curso gratuito de Excel com ${excelCourseStats.modules} módulos, ${excelCourseStats.lessons} aulas em vídeo, atividades práticas e prova final.`,
}

export default async function ExcelCoursePage() {
  ...
  return (
    <main className="site-light-theme min-h-screen">
      ...
      <div className="mx-auto max-w-7xl space-y-10 px-4 py-8">
        <section className="overflow-hidden rounded-3xl border border-violet-200 bg-gradient-to-br from-slate-950 via-violet-950 to-indigo-900 p-7 text-white shadow-2xl shadow-violet-950/20 sm:p-10">
          <div className="grid gap-8 lg:grid-cols-[1.25fr_.75fr] lg:items-center">
            <div>
              <span className="inline-flex items-center gap-2 rounded-full border border-violet-300/30 bg-violet-950/80 px-3 py-1 text-xs font-black uppercase tracking-[.16em] text-violet-100 shadow-sm shadow-black/20">
                <FileSpreadsheet size={16} /> Curso gratuito com cadastro
              </span>
              ...
              <p className="mt-5 max-w-3xl text-base leading-8 text-slate-200 sm:text-lg">
                Aprenda Excel do zero: células, fórmulas, funções essenciais,
                PROCV/PROCX, gráficos e tabelas dinâmicas — com vídeos,
                atividades práticas e prova final.
              </p>

              <div className="mt-6 flex flex-wrap gap-3 text-sm font-bold">
                <span className="inline-flex items-center gap-2 rounded-xl border border-violet-300/30 bg-violet-950/80 px-4 py-2 shadow-sm shadow-black/20">
                  <BookOpenCheck size={18} /> {excelCourseStats.modules} módulos ·{' '}
                  {excelCourseStats.lessons} aulas
                </span>
                <span className="inline-flex items-center gap-2 rounded-xl border border-violet-300/30 bg-violet-950/80 px-4 py-2 shadow-sm shadow-black/20">
                  <Clock3 size={18} /> Carga horária aproximada:{' '}
                  {excelCourseStats.workload}
                </span>
              ...
              <p className="mt-3 text-xs font-bold text-violet-100/80">
                O curso é 100% gratuito. Para emitir o certificado é preciso
                concluir as {excelCourseStats.lessons} aulas e passar na prova
                final (70% de acerto). O certificado é opcional e pago à
                parte.
              </p>
              ...
              {customer ? (
                ...
                <div className="mt-7 flex flex-wrap gap-3">
                  <a
                    className="inline-flex items-center gap-2 rounded-xl bg-violet-400 px-6 py-3 font-black text-slate-950 transition hover:bg-violet-300"
                    href={`/cadastro?next=${encodeURIComponent(firstLessonHref)}`}
                  >
                    <UserPlus size={18} /> Criar conta grátis e começar
                  </a>
                  <a
                    className="inline-flex items-center gap-2 rounded-xl border border-violet-300/30 bg-violet-950/80 px-6 py-3 font-black text-violet-50 shadow-sm shadow-black/20 transition hover:bg-violet-900"
            ...
            <div className="rounded-2xl border border-violet-300/30 bg-violet-950/80 p-5 shadow-lg shadow-black/20">
              <strong className="text-lg text-violet-100">Você vai aprender a:</strong>
              <ul className="mt-4 space-y-3 text-sm leading-6 text-slate-100">
                {[
                  'Criar planilhas e usar fórmulas e funções essenciais.',
                  'Buscar dados entre tabelas com PROCV e PROCX.',
                  'Criar gráficos e tabelas dinâmicas para resumir dados.',
                  'Montar um painel de controle financeiro completo.',
                ].map((item) => (
                  <li className="flex items-start gap-2" key={item}>
                    <CheckCircle2 className="mt-1 shrink-0 text-violet-300" size={17} />
                    {item}
                  </li>
                ))}
              </ul>
                ...
                <div className="mt-5 grid gap-2">
                  {module.lessons.map((lesson, lessonIndex) => (
                    <a
                      ...
                      href={`/curso-excel/aulas/${lesson.slug}`}
                      key={lesson.slug}
                    >
                      <div>
        ...
        <section className="grid gap-5 lg:grid-cols-2">
          <div className="rounded-3xl border border-violet-200 bg-violet-50 p-6">
            ...
            <p className="mt-2 text-sm leading-7 text-slate-700">
              Pessoas que nunca usaram Excel, que querem se organizar melhor
              no trabalho, buscar o primeiro emprego ou aprender a montar
              relatórios e planilhas com mais confiança.
            </p>
          </div>
          ...
          <div className="rounded-3xl border border-amber-200 bg-amber-50 p-6">
            ...
            <p className="mt-2 text-sm leading-7 text-slate-700">
              Depois de concluir as {excelCourseStats.lessons} aulas, você
        ...
        <section className="rounded-3xl border border-violet-200 bg-white/85 p-6">
          ...
          <p className="mt-3 text-sm text-slate-600">
            O conteúdo foi estruturado a partir da documentação oficial do
            Excel.
          </p>
          <a
            className="mt-4 flex items-center justify-between gap-3 rounded-xl border border-slate-200 p-3 text-sm font-bold text-violet-700 transition hover:border-violet-400 hover:bg-violet-50"
            href="https://support.microsoft.com/pt-br/excel"
            rel="noopener noreferrer"
            target="_blank"
          >
            Microsoft — Suporte do Excel <ExternalLink size={16} />
          </a>
        </section>
      </div>
    </main>
