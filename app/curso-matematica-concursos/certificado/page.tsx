...
export default async function MathExamCertificateReturnPage({
  searchParams,
}: {
  searchParams: Promise<{ status?: string }>
}) {
  const { status: statusParam } = await searchParams
  const customer = await requireCustomer('/curso-matematica-concursos/certificado')
  ...
  return (
    <main className="site-light-theme min-h-screen">
      ...
      <div className="mx-auto max-w-2xl px-4 py-16">
        <section className="rounded-3xl border border-indigo-200 bg-white/85 p-8 text-center shadow-sm">
          {certificateStatus.state === 'issued' ? (
            <>
              <CertificatePurchaseTracker
                valueCents={courseRegistry['matematica-concursos'].priceCents}
                verificationCode={certificateStatus.verificationCode}
              />
              <span className="mx-auto grid h-14 w-14 place-items-center rounded-full bg-indigo-600 text-white">
                <CheckCircle2 size={28} />
              </span>
              <h1 className="mt-5 text-2xl font-black text-indigo-950">
              ...
              <a
                className="mt-6 inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-6 py-3 text-sm font-black text-white transition hover:bg-indigo-700"
            ...
            <>
              <span className="mx-auto grid h-14 w-14 place-items-center rounded-full bg-indigo-700 text-white">
                <Clock3 size={28} />
              </span>
              <h1 className="mt-5 text-2xl font-black text-indigo-950">
          ...
          <a
            className="mt-8 inline-block text-sm font-bold text-indigo-700 hover:text-indigo-900"
