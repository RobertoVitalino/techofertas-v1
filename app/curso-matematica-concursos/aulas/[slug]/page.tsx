...
import { mathExamLessons, getMathExamTopic } from '@/lib/math-exam-course'
...
export async function generateMetadata({
  ...
  const topic = getMathExamTopic(slug)
...
export default async function MathExamTopicPage({
  params,
}: {
  params: Promise<{ slug: string }>
}) {
  ...
  const customer = await requireCustomer(
    `/curso-matematica-concursos/aulas/${topic.slug}`,
  )

  const enrolled = await isEnrolledInCourse(customer.id, 'matematica-concursos')
  if (!enrolled) redirect('/curso-matematica-concursos')

  const completedLessons = await getCompletedLessonSlugs(customer.id)

  const currentIndex = mathExamLessons.findIndex(
    (item) => item.slug === topic.slug,
  )
  const previousTopic = mathExamLessons[currentIndex - 1]
  const nextTopic = mathExamLessons[currentIndex + 1]
  const lessonSlugs = mathExamLessons.map((item) => item.slug)
  ...
  return (
    <main className="site-light-theme min-h-screen">
      ...
      <div className="mx-auto max-w-7xl px-4 py-8">
        <a
          className="inline-flex items-center gap-2 text-sm font-black text-indigo-700 hover:text-indigo-900"
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[270px_1fr]">
          <aside className="order-2 space-y-4 lg:order-none lg:sticky lg:top-36 lg:self-start">
            ...
            <nav className="rounded-2xl border border-indigo-200 bg-white/85 p-3 shadow-sm lg:max-h-[62vh] lg:overflow-y-auto">
              <strong className="block px-2 pb-2 text-sm">Tópicos do curso</strong>
              {mathExamLessons.map((item, itemIndex) => (
                <a
                  ...
                  href={`/curso-matematica-concursos/aulas/${item.slug}`}
                  key={item.slug}
                >
          ...
          <article className="order-1 space-y-6 lg:order-none">
            <header className="rounded-3xl border border-indigo-200 bg-gradient-to-br from-white via-indigo-50 to-slate-50 p-6 shadow-sm sm:p-8">
              <div className="flex flex-wrap items-center gap-3 text-xs font-bold text-indigo-700">
                <span className="rounded-full bg-indigo-100 px-3 py-1">
                  Tópico {topic.topicIndex + 1} de {mathExamLessons.length}
                </span>
            ...
            {topicVideo ? (
              <section className="overflow-hidden rounded-2xl border border-indigo-200 bg-black shadow-sm">
            ...
            <section className="grid gap-4">
              {topic.reviewPoints.map((point, index) => (
                <div
                  className="rounded-2xl border border-indigo-200 bg-white/85 p-5 shadow-sm sm:p-6"
                  key={point.title}
                >
                  <div className="flex items-start gap-4">
                    <span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-indigo-100 text-sm font-black text-indigo-800">
                      {index + 1}
                    </span>
                    <div>
            ...
            <nav className="grid gap-3 border-t border-indigo-200 pt-6 sm:grid-cols-2">
              {previousTopic ? (
                <a
                  className="rounded-2xl border border-indigo-200 bg-white/85 p-4 text-sm font-bold transition hover:border-indigo-400 hover:bg-indigo-50"
                  href={`/curso-matematica-concursos/aulas/${previousTopic.slug}`}
                >
              ...
              {nextTopic ? (
                <a
                  className="rounded-2xl border border-indigo-200 bg-indigo-700 p-4 text-sm font-bold text-white transition hover:bg-indigo-800 sm:text-right"
                  href={`/curso-matematica-concursos/aulas/${nextTopic.slug}`}
                >
                  <span className="flex items-center justify-end gap-1 text-xs text-indigo-100">
                ...
                <a
                  ...
                  href="/curso-matematica-concursos/prova-final"
                >
