...
export default async function CustomerAccountPage({
  ...
  const coursesForAccount = [
    ...
    {
      name: 'Curso de Computação Básica',
  ...
  const courseCards = await Promise.all(
    coursesForAccount.map(async (course) => {
      const config = getCourseConfig(course.slug)
      const [lessonsComplete, certificateStatus, examPassed] = await Promise.all([
        isCourseFullyCompleted(customer.id, course.lessonSlugs),
        getCertificateStatus(customer.id, course.slug),
        config.requiresFinalExam
          ? hasPassedFinalExam(customer.id, course.slug)
          : Promise.resolve(true),
      ])
      const priceLabel = (config.priceCents / 100).toLocaleString('pt-BR', {
        minimumFractionDigits: 2,
      })

      return { ...course, config, lessonsComplete, certificateStatus, examPassed, priceLabel }
    }),
  )
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[1.1fr_.9fr]">
          ...
          <div className="grid gap-6">
            {courseCards.map((course) => (
              <section
                className="rounded-3xl border border-sky-400/25 bg-sky-400/10 p-6"
                key={course.slug}
              >
                <div className="flex items-start gap-4">
                  <span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-sky-500/20 text-sky-300"><BookOpenCheck size={22} /></span>
                  <div>
                    <h2 className="text-lg font-black">{course.name}</h2>
                    <p className="mt-1 text-sm leading-relaxed text-slate-400">Acesse as aulas, atividades práticas e revisões usando sua conta.</p>
                    <a href={course.config.landingHref} className="mt-4 inline-flex items-center gap-2 text-sm font-bold text-sky-300 hover:text-white">Continuar o curso <ArrowRight size={16} /></a>

                    <div className="mt-4 border-t border-white/10 pt-4">
                      <p className="text-xs font-bold uppercase tracking-wider text-violet-300">Certificado</p>
                      <p className="mt-1 text-sm leading-relaxed text-slate-400">
                        {course.certificateStatus.state === 'issued'
                          ...
                          : !course.lessonsComplete
                            ...
                            : course.config.requiresFinalExam && !course.examPassed
                              ? 'Aulas concluídas! Falta passar na prova final.'
                              : `Você concluiu o curso! Emita seu certificado por R$ ${course.priceLabel}.`}
                      </p>
                      {course.certificateStatus.state === 'issued' ? (
                        <a href={`/api/certificado/${course.certificateStatus.verificationCode}`} className="mt-2 inline-flex items-center gap-2 text-sm font-bold text-violet-300 hover:text-white">Baixar certificado <Download size={16} /></a>
                      ) : course.lessonsComplete && course.config.requiresFinalExam && !course.examPassed ? (
                        <a href={course.config.examHref} className="mt-2 inline-flex items-center gap-2 text-sm font-bold text-violet-300 hover:text-white">Fazer a prova final <ArrowRight size={16} /></a>
                      ) : (
                        <a href={course.config.landingHref} className="mt-2 inline-flex items-center gap-2 text-sm font-bold text-violet-300 hover:text-white">
                          {course.lessonsComplete ? 'Emitir certificado' : 'Ir para o curso'} <ArrowRight size={16} />
                        </a>
                      )}
                    </div>
                  </div>
